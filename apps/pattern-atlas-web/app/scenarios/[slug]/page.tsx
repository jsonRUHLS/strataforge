...
import { CodeBlock } from "@atlas-patterns/ui";
...
export default async function ScenarioPage({ params }: ScenarioPageProps) {
  ...
  const implementations = scenario.patternLinks.flatMap(({ pattern }) =>
      ...
      <section className="border-t border-white/10 py-8">
        ...
        {implementations.length > 0 ? (
          <div className="mt-4 space-y-4">
            {implementations.map(({ pattern, variant }) => (
              <article
                className="rounded-xl border border-white/10 bg-white/5 p-5"
                key={variant.id}
              >
                ...
                <div className="mt-4">
                  <CodeBlock code={variant.code} language="typescript" />
                </div>
              </article>
            ))}
          </div>
          ...
          <p className="mt-4 max-w-3xl text-slate-300">
            No implementations have been published for the patterns in this
            scenario yet.
          </p>
        )}
      </section>
